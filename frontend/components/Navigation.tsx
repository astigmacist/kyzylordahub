          ...
          <div className="hidden lg:flex items-center gap-3">
            ...
            {mounted && (
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                className="relative w-9 h-9 rounded-full bg-card border border-border flex items-center justify-center text-foreground/70 hover:text-foreground hover:bg-accent transition-all"
              >
                <motion.div
                  initial={false}
                  animate={{
            ...
            {/* Language Switcher */}
            <div className="flex items-center gap-1 px-1 py-1 bg-card border border-border rounded-full">
              <motion.button
                onClick={() => setLocale('ru')}
                whileTap={{ scale: 0.92 }}
                className={`relative px-3 py-1 rounded-full text-xs font-semibold transition-all duration-300 ${locale === 'ru'
                    ? 'text-white'
              ...
              >
                {locale === 'ru' && (
                  <motion.span
                    layoutId="langBg"
                    className="absolute inset-0 rounded-full bg-gradient-to-r from-[#2D6BFF] to-[#FF7A00]"
                    transition={{ type: 'spring', stiffness: 400, damping: 30 }}
                  />
                )}
                <span className="relative z-10">РУС</span>
              </motion.button>
              <motion.button
                onClick={() => setLocale('kz')}
                whileTap={{ scale: 0.92 }}
                className={`relative px-3 py-1 rounded-full text-xs font-semibold transition-all duration-300 ${locale === 'kz'
                    ? 'text-white'
                    : 'text-foreground/50 hover:text-foreground/80'
                  }`}
              >
                ...
                <span className="relative z-10">ҚАЗ</span>
              </motion.button>
            </div>
          </div>
        ...
        >
          <div className="px-6 py-6 space-y-4">
            ...
            <div className="flex items-center gap-2 pt-2">
              <button
                onClick={() => setLocale('ru')}
                className={`px-4 py-2 rounded-full text-xs font-semibold transition-all ${locale === 'ru'
                    ? 'bg-gradient-to-r from-[#2D6BFF] to-[#FF7A00] text-white'
              ...
              <button
                onClick={() => setLocale('kz')}
                className={`px-4 py-2 rounded-full text-xs font-semibold transition-all ${locale === 'kz'
                    ? 'bg-gradient-to-r from-[#2D6BFF] to-[#FF7A00] text-white'
                    : 'bg-card border border-border text-foreground/60'
                  }`}
              >
